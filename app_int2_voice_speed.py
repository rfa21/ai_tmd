...
def initialize_session_state():
    ...
    if 'naver_stt_validated' not in st.session_state:
        st.session_state.naver_stt_validated = False
    ...
    if 'tts_speed' not in st.session_state:
        st.session_state.tts_speed = 1.3  # 기본값 1.3배속
...
def validate_naver_stt(client_id, client_secret):
    """네이버 STT API 키 유효성 검증"""
    try:
        ...
        # HEAD 요청으로 API 접근 가능 여부만 확인
        response = requests.head(url, headers=headers)
        # 401이면 인증 실패, 405나 200대면 API 접근 가능
...
def text_to_speech(text):
    ...
    try:
        ...
        # 속도 조절
        speed_factor = st.session_state.get('tts_speed', 1.3)
...
def clean_text_for_tts(text):
    """TTS를 위한 텍스트 정제 - 이모티콘과 마크다운 제거"""
    import re
    
    # 1. 마크다운 볼드/이탤릭 제거 (내용은 보존)
...
def speech_to_text(audio_bytes, client_id, client_secret):
    """음성을 텍스트로 변환 (네이버 클라우드 CSR 사용)"""
    try:
        url = "https://naveropenapi.apigw.ntruss.com/recog/v1/stt?lang=Kor"
        ...
        if response.status_code == 200:
            ...
            st.error(f"STT API 오류: {response.status_code}")
            return None
...
with st.sidebar:
    ...
    if naver_client_id and naver_client_secret and not st.session_state.naver_stt_validated:
        with st.spinner("네이버 STT API 확인 중..."):
            is_valid, message = validate_naver_stt(naver_client_id, naver_client_secret)
            if is_valid:
                st.session_state.naver_stt_validated = True
                st.session_state.naver_client_id = naver_client_id
                st.session_state.naver_client_secret = naver_client_secret
                st.success("✅ 네이버 STT API 인증됨")
    
    if not st.session_state.naver_stt_validated:
        st.info("💡 [네이버 클라우드 플랫폼](https://console.ncloud.com)에서 API 키 발급")
        
        naver_id_input = st.text_input(
            "Client ID",
            type="password",
        ...
        naver_secret_input = st.text_input(
            "Client Secret",
            type="password",
        ...
        if st.button("🔑 네이버 STT API 인증", use_container_width=True, key="naver_auth_btn"):
            if naver_id_input and naver_secret_input:
                with st.spinner("API 키 검증 중..."):
                    is_valid, message = validate_naver_stt(naver_id_input, naver_secret_input)
                    if is_valid:
                        st.session_state.naver_stt_validated = True
                        st.session_state.naver_client_id = naver_id_input
                        st.session_state.naver_client_secret = naver_secret_input
                        st.success("✅ 네이버 STT API 인증됨")
                        st.rerun()
                    else:
                        st.error(f"❌ API 키가 유효하지 않습니다.\n\n오류: {message}")
            else:
                st.warning("⚠️ Client ID와 Secret을 모두 입력해주세요.")
    else:
        st.success("✅ 네이버 STT API 연결됨")
        if st.button("🔄 네이버 API 재설정", key="reset_naver"):
            st.session_state.naver_stt_validated = False
            if 'naver_client_id' in st.session_state:
                del st.session_state.naver_client_id
            if 'naver_client_secret' in st.session_state:
                del st.session_state.naver_client_secret
            st.rerun()
    
    # 음성 모드 토글
    if st.session_state.api_key_validated and st.session_state.naver_stt_validated:
        st.markdown("---")
        st.subheader("🔊 음성 대화 모드")
        voice_mode = st.toggle("음성 모드 활성화", value=st.session_state.voice_mode, key="voice_toggle")
        ...
        if st.session_state.voice_mode:
            st.success("🔊 음성 모드 활성화됨")
            st.caption("AI의 응답이 자동으로 음성으로 재생됩니다.")
            
            # TTS 속도 조절 슬라이더
            st.markdown("---")
            st.subheader("🎵 TTS 속도 조절")
            ...
            speed_factor = st.slider(
                ...
                step=0.1,
                help="1.0 = 보통 속도, 1.3 = 빠르게, 0.8 = 느리게",
                key="tts_speed_slider"
            )
            
            # 속도 변경 시 세션 상태 업데이트
            if speed_factor != st.session_state.tts_speed:
                st.session_state.tts_speed = speed_factor
            
            # 현재 속도 표시
            if speed_factor == 1.0:
                st.caption("⏯️ 보통 속도")
            elif speed_factor < 1.0:
                st.caption(f"🐢 느리게 ({speed_factor}배속)")
            else:
                st.caption(f"🐰 빠르게 ({speed_factor}배속)")
        else:
            st.info("💬 텍스트 모드")
...
# ========== 메인 화면 ==========
st.title("🦷 턱관절 AI 대화형 음성 문진 시스템")

# 음성 모드 상태 표시
if st.session_state.voice_mode:
    st.success("🔊 **음성 대화 모드 활성화** - AI 응답이 자동으로 음성으로 재생됩니다")
else:
    st.info("💬 **텍스트 대화 모드** - 사이드바에서 음성 모드를 활성화할 수 있습니다")
...
if st.session_state.pending_tts:
    st.info("🔊 AI 응답을 음성으로 재생합니다...")
    audio_file = text_to_speech(st.session_state.pending_tts)
    if audio_file:
        autoplay_audio(audio_file)
...
if not st.session_state.conversation_complete:
    ...
    if st.session_state.voice_mode:
        ...
        with col1:
            ...
            # 녹음 시작 감지 (오디오가 새로 생성되면 TTS 중단)
            if audio_bytes:
                current_len = len(audio_bytes)
                if current_len != st.session_state.last_audio_len:
                    # 새 녹음 시작 - pending TTS 제거 (재생 중단)
                    st.session_state.pending_tts = None
        
        with col2:
            st.caption("또는")
            text_input_mode = st.checkbox("텍스트로 입력", key="text_input_toggle")
        
        if audio_bytes and not text_input_mode:
            # 새로운 녹음인지 확인 (길이로 비교)
            current_audio_len = len(audio_bytes)
            
            if current_audio_len != st.session_state.last_audio_len:
                # 새 녹음!
                ...
                if transcribed_text:
                    ...
                    with st.spinner("💭 답변 생성 중..."):
                        response, is_json, progress = call_claude(transcribed_text)
